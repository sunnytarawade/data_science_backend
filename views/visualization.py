...
@visualization_blueprint.route('/duration_distribution/<path:upload_id>/<int:no_of_duration_intervals>/', methods=['GET'])
def get_duration_distribution_data(upload_id,no_of_duration_intervals):
    data_file_path = os.path.join(UPLOAD_FOLDER,upload_id,'rides_data_file_cleaned.csv')
    rides_data = pd.read_csv(data_file_path)

    duration_ranges_dict = pd.cut(rides_data['duration'], no_of_duration_intervals).value_counts().sort_index().to_dict()
...
@visualization_blueprint.route('/popular_stations/<path:upload_id>/<int:no_of_stations>/', methods=['GET'])
def get_popular_station_map_data(upload_id,no_of_stations):
    ...
    most_popular_stations_ids = list(sorted_station_frequency_dict.keys())[:no_of_stations]
